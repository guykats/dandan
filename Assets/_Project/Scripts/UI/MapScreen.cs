...
/// <summary>
/// Builds the full map screen at runtime — no prefabs, no serialized refs.
/// Visual style: sky gradient, dotted path, circular level nodes with shadow + glow.
/// </summary>
public class MapScreen : MonoBehaviour
{
    ...
    // ── Node layout ──────────────────────────────────────────────────────────
    // Normalized (0-1) within the map area (y=0 bottom, y=1 top)
    ...
    // Reference size of the map content area (matches CanvasScaler reference)
    ...
    {
        // 3 overlapping ovals = a cloud puff
    ...
    void BuildNode(GameObject map, int idx)
    {
        ...
        // Main circle — this is the button hit target
        ...
        // Inner highlight — top-left gleam for 3D ball feel
        ...
        var bTxtGO = TMP(bGO, "Op", opStr, 32f, FontStyles.Bold,
                         col, TextAlignmentOptions.Center);
        Stretch(bTxtGO, Vector2.zero, Vector2.one);

        // Stars row below circle
        ...
        // Operation type label (e.g. "חיבור" / "חיסור")
    ...
    IEnumerator PulseGlow()
    {
        ...
        while (true)
        {
            ...
            float scale = 1f + Mathf.Sin(t) * 0.07f;
            _glowRT.sizeDelta = Vector2.one * _glowBase * scale;
            var c = _glowImg.color;
            _glowImg.color = new Color(c.r, c.g, c.b, 0.26f + Mathf.Sin(t) * 0.18f);
            yield return null;
        }
    }
    ...
    void OnLevelClicked(int level)
    {
        ...
        // TODO: load game screen for this level
    }
